...
const AllSellers = () => {
    const { data: sellers = [], refetch, isLoading } = useQuery({
        queryKey: ['sellers'],
        queryFn: () => fetch('https://buy-sell-store-backend.vercel.app/sellers', {
            headers: {
    ...
    const handleDelete = (id) => {
        console.log(id)
        fetch(`https://buy-sell-store-backend.vercel.app/sellers/${id}`, {
            method: "DELETE",
            headers: {
        ...
        <div className="overflow-x-auto w-full">
            <table className="table w-full">
                ...
                <tbody>
                    ...
                    {
                        sellers?.length > 0 && sellers.map((seller, i) => <tr key={seller._id}>
                            <th>
                                {i + 1}
                            </th>
                            <td>{seller.name}</td>
